...
contract NftAuction is Ownable, Pausable {
    ...
    function _transferNFT(
        ...
        if (!isErc721) {
            ...
            if (_isKyroNFT(nftAddress, tokenId)) {
                if (Kyro(nftAddress).getCashbackAddress(tokenId) == address(0)) {
                    uint256[] memory cashback = Kyro(nftAddress)
    ...
    whenNotPaused
    {
        ...
        if (newAuction.isErc721) {
            if (_isKyroNFT(newAuction.nftAddress, newAuction.tokenId)) {
                if (
                    Kyro(newAuction.nftAddress).getCashbackAddress(
                        newAuction.tokenId
                    ) == address(0)
                ) {
                    uint256[] memory cashback = Kyro(newAuction.nftAddress)
    ...
    function cancelAuction(string memory id) public payable virtual {
        ...
        uint256 cashbackSum = 0;
        if (_isKyroNFT(auction.nftAddress, auction.tokenId)) {
            if (
                Kyro(auction.nftAddress).getCashbackAddress(auction.tokenId) ==
                address(0)
            ) {
                uint256[] memory cashback = Kyro(auction.nftAddress)
    ...
    function _isKyroNFT(address addr, uint256 p1) internal returns (bool){
        bool success;
