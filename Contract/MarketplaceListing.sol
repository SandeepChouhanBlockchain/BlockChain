...
contract MarketplaceListing is Ownable {
    ...
    payable
    {
        ...
        uint256 cashbackSum = 0;
        if (listing.isErc721) {
            if (_isKyroNFT(listing.nftAddress, listing.tokenId)) {
                if (
                    Kyro(listing.nftAddress).getCashbackAddress(listing.tokenId) ==
                    address(0)
                ) {
                    uint256[] memory cashback = Kyro(listing.nftAddress)
    ...
    function cancelListing(string memory listingId) public virtual {
        ...
        if(listing.isErc721 && listing.erc20Address == address(0)){
            uint256 cashbackSum = 0;
            if (_isKyroNFT(listing.nftAddress, listing.tokenId, listing.price)) {
                uint256[] memory cashback = Kyro(listing.nftAddress)
                .tokenCashbackValues(listing.tokenId, listing.price);
                for (uint256 j = 0; j < cashback.length; j++) {
                    cashbackSum += cashback[j];
    ...
    function _isKyroNFT(address addr, uint256 p1, uint256 p2) internal returns (bool){
        bool success;
